...
> extends React.Component<Props<I, C, T>, State> {
  ...
  private _getVisualListDuringDrag(): readonly I[] {
    const { list } = this.props
    const { dragging, lastDrag } = this.state
  ...
  private _getDistance(
    ...
    dragging: boolean,
    list: readonly I[] = this.props.list
  ): number {
    if (end < start) {
    ...
    return (
      <div style={{
        ...
        <Motion
          ...
          children={({ adjustScroll }: { adjustScroll: number }) => (
            <div
              style={{
